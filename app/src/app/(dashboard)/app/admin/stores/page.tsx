import { Bot, CreditCard, ExternalLink, Search, Store } from "lucide-react";
...
function AdminStoreActions({ storeId, planCode, publicUrl }: { storeId: string; planCode: string; publicUrl: string }) {
  return (
    <div className="space-y-2">
      ...
      <Link href={`/app/admin/payments?storeId=${encodeURIComponent(storeId)}`} className="inline-flex h-10 w-full items-center justify-center gap-2 rounded-md border border-brand-border bg-white px-3 text-xs font-black text-brand-dark hover:border-brand">
    ...
    <section className="space-y-4 md:space-y-6">
      ...
      <div className="space-y-3">
        {rows.length === 0 ? (
          ...
          rows.map(({ store, productUsage, sellerAiUsage, planState, paymentSummary, leadSignals }) => {
              ...
              <article key={store.id} className="rounded-lg border border-brand-border bg-brand-paper p-4 shadow-sm md:p-5">
                <div className="grid gap-4 xl:grid-cols-[minmax(0,1.15fr)_minmax(0,0.9fr)_minmax(260px,0.7fr)]">
                  ...
                  <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-1">
                    ...
                    <div className="rounded-md bg-brand-muted px-3 py-2">
                      <p className="text-[11px] font-black uppercase text-neutral-500">Pagos</p>
                      <p className="mt-1 text-sm font-black text-brand-dark">{formatStorePaymentMoney(paymentSummary.confirmedTotalCents, store.currency)}</p>
                      {paymentSummary.lastPayment ? (
                        <span className={cn("mt-1 inline-flex rounded-full px-2.5 py-1 text-xs font-black", paymentStatusClass(paymentSummary.lastPayment.status))}>
                          Último: {storePaymentStatusLabel(paymentSummary.lastPayment.status)}
                        </span>
                      ) : (
                        <p className="mt-1 text-xs font-semibold text-neutral-500">Sin pagos registrados</p>
                      )}
                    </div>
                  </div>
