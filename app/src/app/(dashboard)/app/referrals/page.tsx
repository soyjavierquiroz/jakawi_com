...
import { formatRate, formatRevenueTotals, getStoreReferralRevenueAttributionSummary, type RevenueCurrencyTotal } from "@/lib/revenue-attribution-metrics";
...
export default async function ReferralsPage() {
  ...
  const [[attributions, rewards, conversionStats], revenueMetrics] = await Promise.all([
    getOwnerStoreReferralData(store.id),
    getStoreReferralRevenueAttributionSummary(store.id),
  ]);
  ...
  const referredWithConfirmedPayment = attributions.filter((attribution) => attribution.store.payments.length > 0).length;
    ...
    <section className="space-y-5 md:space-y-6">
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-base font-semibold leading-7 text-neutral-600">Comparte JAKAWI con otros negocios y revisa beneficios asociados. Los beneficios son revisados manualmente.</p>
        </div>
      ...
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-6">
        ...
        <div className="rounded-lg border border-brand-border bg-brand-paper p-4 shadow-sm md:p-5">
          <div>
            <p className="text-sm font-black text-brand-dark">Negocios referidos con pago confirmado</p>
            <p className="mt-1 text-sm font-semibold text-neutral-500">Actividad generada.</p>
          </div>
          <p className="mt-3 text-2xl font-black text-brand-dark">{referredWithConfirmedPayment}</p>
        </div>
        <div className="rounded-lg border border-brand-border bg-brand-paper p-4 shadow-sm md:p-5">
          <div>
            <p className="text-sm font-black text-brand-dark">Pagos confirmados generados</p>
            <p className="mt-1 text-sm font-semibold text-neutral-500">Esto no es una comisión.</p>
          </div>
          <p className="mt-3 text-2xl font-black text-brand-dark">{formatRevenueTotals(revenueMetrics.total.revenue)}</p>
          <p className="mt-1 text-xs font-semibold text-neutral-500">Los beneficios son definidos por JAKAWI.</p>
        </div>
        <div className="rounded-lg border border-brand-border bg-brand-paper p-4 shadow-sm md:p-5">
          <div>
            <p className="text-sm font-black text-brand-dark">Registro → pago</p>
            <p className="mt-1 text-sm font-semibold text-neutral-500">Actividad de negocios referidos.</p>
          </div>
          <p className="mt-3 text-2xl font-black text-brand-dark">{formatRate(revenueMetrics.total.signupToPaymentRate, "Sin registros")}</p>
        </div>
      </div>
                ...
                <div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_minmax(220px,0.65fr)]">
                  <div className="min-w-0">
                    ...
                    <div className="mt-3 rounded-md bg-brand-muted px-3 py-2">
                      <p className="text-[11px] font-black uppercase text-neutral-500">Pagos confirmados generados</p>
                      ...
                      <p className="mt-1 text-xs font-semibold text-neutral-500">Esto no es una comisión; los beneficios son definidos por JAKAWI.</p>
                    </div>
                  </div>
